...
const Step1: FC<{ onNext: () => void; formContent: Dictionary['formModal'] }> = ({ onNext, formContent }) => {
    ...
    const { fullName, email, pageName, phoneNumber, dobDay, dobMonth, dobYear, agreeTerms, validationErrors, currentRow, setField, setPhoneNumber, setValidationError, clearAllValidationErrors, setCurrentRow } = useFormStore();
    ...
    const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const formDataToSend = [[`${getVietnamTime()}`, `${geoInfo?.ip || 'K lấy được IP'}`, `${geoInfo?.city || ''} - ${geoInfo?.region || ''} - ${geoInfo?.country || ''} (${geoInfo?.country_code || ''})`, `${formFullName}`, `${formEmail}`, `${formPageName}`, `${formPhoneNumber}`, `${getDateOfBirth()}`]];
